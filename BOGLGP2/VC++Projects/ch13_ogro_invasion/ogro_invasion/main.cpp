...
#endif
{
    ...
    if (MessageBox(NULL, L"Would you like to run in fullscreen mode?", L"Fullscreen?", MB_ICONINFORMATION | MB_YESNO) == IDYES)
    {
    ...
    {
#ifdef _WIN32
        MessageBox(NULL, L"Could not initialize the application", L"An error occurred", MB_ICONERROR | MB_OK);
#endif
        programWindow.destroy(); //Reset the display and exit
        return 1;
    }
